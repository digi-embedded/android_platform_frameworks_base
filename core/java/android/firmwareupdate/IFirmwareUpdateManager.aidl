...
/** @hide */
interface IFirmwareUpdateManager
{
    /* Installs the given update package. */
    void installUpdatePackage(String packagePath, IFirmwareUpdateListener listener, boolean verify, String deviceCertsZipFile);

    /* Wipes the cache partition. */
    Bundle wipeCache(String reason);

    /* Wipes the user data partition. */
    Bundle wipeUserData(String reason, boolean shutdown);

    /* Installs the given application. */
    Bundle installApplication(String applicationPath, boolean reboot);

    /* For reporting progress callback completion */
    void listenerCallbackFinished(IFirmwareUpdateListener listener);
}
